...
def login():
    try:
        sql ="select * from app_ppd_spider where url_home is  null  and platform_id ='ppd' order by grade LIMIT 5,1"
        data = con_mysql(sql)
        #获取查询到的手机号进行去重操作
        id = data[0][0]
        mobile = data[0][1]
        ...
        if data_w[0][0] == 0  and len(black_data)==0 and len(week_data) == 0:
            for i in range(len(data)):
             ...
             #     status_yqb, url_home, url_detail = req_yqb(uname, pw)
             #     if status_yqb ==2 or status_yqb ==3:
             #         flag =False
            ...
            #回调
             ...
             # code = '200'
             # white_code = '200'
             sql = "update app_ppd_spider set url_home='" + url_home_ppd + "'," \
                    ",url_detail='" + url_detail_ppd + "',back_code='" + code + "'," \
            "status='" + str(status_ppd) + "'white_code='" + white_code + "', update_date=now() where id='" + id + "'"
             print('更新数据库 %s ' % sql)
             con_mysql(sql)
...
def req_yqb(uname,pw):
    ...
    if status == 1:
        status = read_img_yqb(address_detail, 1)
