...
def ocr_api(image_url):
    ...
    for i in range(0,word_length):
        ...
        if word1 =="还款":
            ...
            date = word_result[i - 2]["words"]
            if date.count(".") > 0:
                print("下一期还款时间 =====" + word_result[i - 2]["words"])
                loan_start_date, loan_last_date = get_date_1(date, qs)
            else:
                print("下一期还款时间 =====" + word_result[i - 2]["words"])
                date = word_result[i - 1]["words"]
                loan_start_date, loan_last_date = get_date_1(date, qs)
            # loan_start_date,loan_last_date = get_date(date,qs)
            print("开始借款时间-----"+loan_start_date)
            print("最后一期还款时间------"+loan_last_date)
    if loan_start_date =="" or loan_last_date=="":
        for i in range(0, word_length):
            word1 = str(word_result[i]["words"])
            if word1 == "已还清":
                ...
                date = word_result[i - 2]["words"]
                if date.count(".") > 0:
                    print("第一期期还款时间 =====" + word_result[i - 2]["words"])
                    loan_start_date, loan_last_date = get_date_1(date, qs)
                else:
                    print("第一期期还款时间 =====" + word_result[i - 2]["words"])
                    date = word_result[i - 1]["words"]
...
def get_date_1(date,qs):
    year, month, day = date.split(".")
    month = int(month)
    start_month = month - 1
    back_month = month + qs -1
    if start_month == 0:
        year = int(year) - 1
        start_date = str(year) + "-12-" + str(day)
    elif start_month < 10:
    ...
    if back_month >12 :
        ...
        last_date = str(year) + "-0" + str(back_month) + "-" + str(day)
    return   start_date,last_date
image_url = 'http://123.dev.zhishensoft.com:58008/images/ppd/20180718/20180718100710.png'
ocr_api(image_url)